...
Из полученных 10 значений отбрасывал минимальное и максимальное (для устранения случайных выбросов). Для оставшихся 8 значений вычислял: Среднее арифметическое, Стандартное отклонение (погрешоность) (несмещённая оценка, делитель `n−1 = 7`).
...
Во время тестов с помощью AIDA64 велся мониторинг частоты ядра CPU3 и температуры процессора.  
Графики, представленные в таблице 1, соответствуют конфигурациям с флагом `-O2`.  
- Частота ядра остаётся стабильной (колебания в пределах 3200–3400 МГц).  
- Температура не превышает 75-80 °C.  
Данные подтверждают отсутствие троттлинга и стабильность условий работы программы.
...
- Немедленно `vmovss DWORD PTR [rsp-80], xmm9` – сохранение результата в стек;
...
1. Из этой работы можно сделать следующие выводы: автоматическая векторизация компилятором может превзойти по эффективности код с AVX2‑интринсиками, если структура программы дает компилятору «понять» что хотел сделать программист.
2. Важный "урок" `-O3` не всегда даёт прирост производительности; иногда его агрессивная оптимизация вредна и приводит к **деоптимизации**, поэтому выбор флагов должен подтверждаться измерениями.
3. Ручная векторизация (AVX2) обеспечивает стабильное и предсказуемое ускорение, но требует знание интринсиков.
4. Также было произведено сравнение с предыдущим отчетом, результаты получились вполне воспроизводимые, с относительной погрешностью от 0.02% до 0.5%.
